...
ruta_origen = Path(r'C:\Users\apr.gestiondered6\Music\Resumen Estad Proy. Viab.xlsx')
ruta_destino = Path(r'C:\Users\apr.gestiondered6\Music\Análisis Generación_VF.xlsx')
ruta_base = Path(r'C:\Users\apr.gestiondered6\OneDrive - Air-e SAS ESP\ESTADO DE PROYECTOS PARA VIABILIDAD WT')

excel = pd.read_excel(ruta_origen, header=3)  # Datos principales
excel_destino = pd.read_excel(ruta_destino, header=4)  # Datos de destino (desde fila 5)
...
for i in range(cantidad_proyectos):
    try:
        ...
        try:
            df_inspeccion = pd.read_excel(archivo_excel, sheet_name='INSPECCIÓN')
        ...
        for idx, fila in df_inspeccion.iterrows():
            ...
            for col, val in mappings.items():
                if pd.isna(excel_destino.at[index_destino, col]):
...
try:
    with pd.ExcelWriter(ruta_destino, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
        excel_destino.to_excel(writer, index=False, header=False, startrow=5)  # Sobrescribir desde fila 6
